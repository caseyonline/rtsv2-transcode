...
# /nix/store/1b0r4jgbm5sk2wnh5cicbhj0q281pldc-ffmpeg-3.4.6-bin/bin/ffmpeg -re -i "$file" -ar 48000 -acodec aac -vcodec copy -bf 0 -f flv "$ingestUrl pubUser=user pubPasswd=password"
...
ffmpeg \
  ...
  -bf 0 \
  -f flv "$ingestUrl pubUser=user pubPasswd=password"

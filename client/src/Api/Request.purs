...
-- | functions that our production monad will leverage in `Rtsv2App.AppM` to implement our various
-- | app capabilities.
...
-- | Rtsv2App uses a REST API and secures certain endpoints using a JSON Web Token (JWT). That means
...
-- |
-- | Check out the `Rtsv2App.Capability.Resource.*` modules for examples of the high-level functions
...
-- | request functions defined below and in the `Rtsv2App.Api.Utils` modules.
...
-- |
-- | Both functions -- as well as functions from the `Rtsv2App.Capability.Resource.User` module -- 
...
-- | However, in the `Rtsv2App.Capability.Resource.User` module, we'll see another record using  
...
-- | particular key. They'll be used as part of our production monad, `Rtsv2App.AppM`.

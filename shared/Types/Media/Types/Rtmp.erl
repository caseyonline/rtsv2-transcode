-module(shared_types_media_types_rtmp@foreign).

-export([
         foreignToPursImpl/1
        ]).

foreignToPursImpl(_DictMonoid) ->
  fun(Metadata, Map, Append) ->
      lists:foldl(fun({Name, Value}, Acc) ->
                      add_value_to_map(Name, Value, Acc, Append)
                  end,
                  Map,
                  Metadata)
  end.
...
%% Audio
add_value_to_map(<<"audiocodecid">>, Value, Map = #{audio := Audio}, _Append) ->
  maps:put(audio, maps:put(codecId, {just, map_audio_codec_id(Value)}, Audio), Map);

add_value_to_map(<<"audiodatarate">>, Value, Map = #{audio := Audio}, _Append) ->
  maps:put(audio, maps:put(dataRate, {just, Value}, Audio), Map);

add_value_to_map(<<"audiosamplesize">>, Value, Map = #{audio := Audio}, _Append) ->
  maps:put(audio, maps:put(sampleSize, {just, trunc(Value)}, Audio), Map);

add_value_to_map(<<"audiosamplerate">>, Value, Map = #{audio := Audio}, _Append) ->
  maps:put(audio, maps:put(sampleRate, {just, trunc(Value)}, Audio), Map);

add_value_to_map(<<"stereo">>, Value, Map = #{audio := Audio}, _Append) ->
  maps:put(audio, maps:put(stereo, {just, Value}, Audio), Map);

add_value_to_map(<<"audiodevice">>, Value, Map = #{audio := Audio}, _Append) ->
  maps:put(audio, maps:put(device, {just, Value}, Audio), Map);

add_value_to_map(<<"audiochannels">>, Value, Map = #{audio := Audio}, _Append) ->
  maps:put(audio, maps:put(channels, {just, trunc(Value)}, Audio), Map);

add_value_to_map(<<"audioinputvolume">>, Value, Map = #{audio := Audio}, _Append) ->
  maps:put(audio, maps:put(inputVolume, {just, Value}, Audio), Map);

%% Video
add_value_to_map(<<"videocodecid">>, Value, Map = #{video := Video}, _Append) ->
  maps:put(video, maps:put(videoCodecId, {just, map_video_codec_id(Value)}, Video), Map);

add_value_to_map(<<"videodatarate">>, Value, Map = #{video := Video}, _Append) ->
  maps:put(video, maps:put(videoDataRate, {just, Value}, Video), Map);

add_value_to_map(<<"framerate">>, Value, Map = #{video := Video}, _Append) ->
  maps:put(video, maps:put(frameRate, {just, Value}, Video), Map);

add_value_to_map(<<"width">>, Value, Map = #{video := Video}, _Append) ->
  maps:put(video, maps:put(width, {just, trunc(Value)}, Video), Map);

add_value_to_map(<<"height">>, Value, Map = #{video := Video}, _Append) ->
  maps:put(video, maps:put(height, {just, trunc(Value)}, Video), Map);

add_value_to_map(<<"videodevice">>, Value, Map = #{video := Video}, _Append) ->
  maps:put(video, maps:put(device, {just, Value}, Video), Map);

add_value_to_map(<<"videokeyframe_frequency">>, Value, Map = #{video := Video}, _Append) ->
  maps:put(video, maps:put(keyFrameFrequency, {just, Value}, Video), Map);

add_value_to_map(<<"avcprofile">>, Value, Map = #{video := Video}, _Append) ->
  maps:put(video, maps:put(avcProfile, {just, map_avc_profile(Value)}, Video), Map);

add_value_to_map(<<"avclevel">>, Value, Map = #{video := Video}, _Append) ->
  maps:put(video, maps:put(avcLevel, {just, Value}, Video), Map);

%% Misc
add_value_to_map(<<"duration">>, Value, Map, _Append) ->
  maps:put(duration, {just, Value}, Map);

add_value_to_map(<<"encoder">>, Value, Map, _Append) ->
  maps:put(encoder, {just, Value}, Map);

add_value_to_map(<<"filesize">>, Value, Map, _Append) ->
  maps:put(filesize, {just, Value}, Map);

add_value_to_map(Name, Value, Map = #{other := Other}, Append) ->
  maps:put(other, Append([#{name => Name,

...
If either of those assumptions is untrued, the code of `rtsv2_egest_stress_test.erl`
...
queue capacity is 500Mbit, and the stream is 100Mbit, 499 clients will be scheduled on to a single transmit queue (our scheduling is conservative), so ~ 497 virtual clients should leave room for the two browser clients used by the test.
